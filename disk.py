def loadinventory():
    ...
    for item in characters:
        ...
        dictionary = {key_1: Brand, key_2: Character, key_3: float(Stock), key_4: float(Price), key_5: float(Rented), key_6: float(Value)}
...
def update_inventory(menu):
    ...
    for item in menu:
        message += '{}, {}, {}, {:.2f}, {}, {:.2f}\n'.format(item["Brand"], item["Character"], float(item["Stock"]), float(item["Price"]), float(item["Rented"]), float(item["Value"]))
